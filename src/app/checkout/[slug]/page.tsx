...
export default function Checkout({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-slate-700">Placeholder checkout. Selanjutnya kita hubungkan ke Midtrans Snap/VT-Web.</p>
      <div className="p-4 border rounded-2xl">
        <div className="flex items-center justify-between">
          <div>{svc.title}</div>
          {svc.price ? <div className="font-semibold">Rp {svc.price.toLocaleString("id-ID")}</div> : null}
        </div>
      </div>
